<template>
  <view class="pages order-create">
    <van-tabs type="card" color="#f85437" :active="deliveryTypeIndex" @change="changeDeliveryType" custom-class="order-type-tabs">
      <van-tab title="送货上门">
        <view class="order-customer-address">
          ...
          <!-- 超出配送范围 -->
          <view class="order-address-exceed" v-if="orderAddressInfo.info.id&&!orderAddressInfo.distance">
            <van-icon custom-class="icon-warning" name="bulb-o" size="14px" color="#ff976a" />
            <text>当前地址超出店铺配送范围，请更改地址或直接上门自提！</text>
          </view>
      ...
      <van-tab title="到店自提">
        <view class="order-store-address">
          <view class="order-store-address-title">商家地址</view>
          <view class="order-store-address-content">{{storeInfo.shopAddress}} </view>
          <view class="order-store-address-detail"><text> | </text><text class="see-map" @click="seeMap">查看地图</text></view>
          <view class="order-store-address-footer">
            <view class="self-mention-left">
              <view class="self-mention-title">自提时间</view>
              <view class="self-mention-time"><text>{{selfLiftingTime}}</text></view>
              <view class="self-mention-time-info">自提前需致电商家确认</view>
            </view>
            <view class="self-mention-right">
              <view class="self-mention-title">手机号码</view>
              <view class="self-mention-mobile">
                <van-field required is-link :value="selfMentionMobile" type="number" maxlength="11" placeholder="请输入手机号码" @change="bindSelfMentionMobile" />
              </view>
            </view>
          </view>
    ...
    <view class="order-goods">
      <view class="order-item">
        ...
        <view class="order-money-info">
          ...
          <view class="order-item-money" v-if="deliveryType==='home'">
          ...
          <view class="order-item-money" v-if="deliveryType==='home'&&sendHomeTime">
            <view class="order-item-money-title">配送时间：</view>
            <view class="order-item-money-value">{{sendHomeTime}}</view>
          </view>
        </view>
...
export default {
  ...
  data() {
    return {
      ...
      groupId: null, //团ID
      source: null,
      orderType: "onlineOrder", //onlineOrder：线上订单；
      saleType: null, // group：拼团；seckill：秒杀
      deliveryTypeIndex: 0,
      deliveryType: "home", //'home':送货上门；'shop':到店自提

      hasAddress: !1, //是否存在地址
      storeInfo: {},
      sendHomeTime: "0:00-24:00", // 送货上门时间
      selfLiftingTime: "0:00-24:00", // 自提时间
      selfMentionMobile: null, // 自提手机号
      orderGoodsList: [], // 商品信息
      goodsMoney: 0, // 商品总金额
  ...
  methods: {
    ...
    //绑定到店自提手机号
    bindSelfMentionMobile(e) {
      this.selfMentionMobile = e.mp.detail;
    },
    ...
    //查看地图
    seeMap() {
      const latitude = this.storeInfo.shopLatitude;
      const longitude = this.storeInfo.shopLongitude;
      const name = this.storeInfo.shopName;
      const address = this.storeInfo.shopAddress;
      wx.openLocation({
        latitude,
        longitude,
        name,
    ...
    async getOrderInfo() {
      ...
      const info = await api
        ...
        .then(res => {
          ...
          this.selfMentionMobile = data.pickPhoneNumber;
          this.storeInfo = data.shopInfo;

          //店铺配送时间
          if (data.shopInfo.deliveryState == "disabled") {
            this.sendHomeTime = "暂停配送";
          } else if (data.shopInfo.deliveryState == "enabled") {
            if (data.shopInfo.deliveryHours) {
              const _time = data.shopInfo.deliveryHours.split(",");
              this.sendHomeTime = `${_time[0]}-${_time[1]}`;
            } else {
              this.sendHomeTime = "0:00-24:00";
            }
          } else {
            const _time = data.shopInfo.deliveryState.split(",");
            this.selfLiftingTime = `${_time[0]}-${_time[1]}`;
          }

          // 自提时间
          if (data.shopInfo.businessState == "disabled") {
            this.selfLiftingTime = "店铺关门了";
          } else if (data.shopInfo.businessState == "enabled") {
            if (data.shopInfo.businessHours) {
              const _time = data.shopInfo.businessHours.split(",");
              this.selfLiftingTime = `${_time[0]}-${_time[1]}`;
            } else {
              this.selfLiftingTime = "0:00-24:00";
            }
          } else {
            const _time = data.shopInfo.businessState.split(",");
            this.selfLiftingTime = `${_time[0]}-${_time[1]}`;
          }
          ...
          if (!this.saleType) {
            ...
            //积分抵扣金额
            // if (data.pointsDeduction > 0) this.isUseIntegral = !0;
            // else this.isUseIntegral = !1;
    ...
    calcPaymentMoney() {
      ...
      if (this.deliveryType === "home") {
        if (this.isFreeDeliveryMoney) {
          this.finalPaymentMoney = a - c - d - e;
        } else {
          this.finalPaymentMoney = a + b - c - d - e;
        }
      } else if (this.deliveryType === "shop") {
        this.finalPaymentMoney = a - c - d - e;
      }
    },
    ...
    checkOrderData() {
      ...
      if (this.storeInfo.businessState == "disabled") {
        util.showToastError("店铺暂停营业中！");
        flag = !1;
        return flag;
      }
      ...
      if (this.deliveryType === "home") {
        if (!this.orderAddressInfo.info.id) {
          ...
          if (!this.orderAddressInfo.distance) {
            Notify({
              text: "当前地址超出店铺配送范围，请更改地址或直接上门自提！"
            });
            flag = !1;
            return flag;
          }
        ...
        if (this.storeInfo.deliveryState == "disabled") {
          Dialog.confirm({
            title: "提示",
            message: "店铺已暂停配送，是否自动转为自提订单？"
          })
            .then(() => {
              this.deliveryType = "shop";
              this.deliveryTypeIndex = 1;
              this.calcPaymentMoney();
            })
            .catch(() => {});
          flag = !1;
        ...
        if (
          this.storeInfo.deliveryState == "enabled" &&
          this.sendHomeTime != "0:00-24:00"
        ) {
          const _sendHomeTime = this.sendHomeTime;
          const _sendHomeTimeStart = `${_sendHomeTime.split("-")[0]}:00`;
          const _sendHomeTimeEnd = `${_sendHomeTime.split("-")[1]}:00`;
          let curDate = new Date();
          let year = curDate.getFullYear();
          let month = curDate.getMonth() + 1;
          let day = curDate.getDate();

          const startDate = `${year}/${month}/${day} ${_sendHomeTimeStart}`;
          const endDate = `${year}/${month}/${day} ${_sendHomeTimeEnd}`;
          ...
          if (curTimeStamp < startTimeStamp || curTimeStamp > endTimeStamp) {
            Dialog.confirm({
              title: "提示",
              message: "当前时间不在店铺配送时间范围内，是否自动转为自提订单？"
            })
              .then(() => {
                this.deliveryType = "shop";
                this.deliveryTypeIndex = 1;
        ...
        if (!this.selfMentionMobile) {
          util.showToastError("请输入手机号！");
          flag = !1;
          return flag;
        }
      }
    ...
    async createOrder(data) {
      const order = await api
        ...
        .then(res => {
          ...
          if (this.source == "cart") this.removeCartGoodsItem();
          util.redirectTo(url);
        })
        .catch(err => {
          console.log(err);
    ...
    removeCartGoodsItem() {
      ...
      const idsNumArr = this.idsNum.split(",");
      let idsArr = [];
      idsNumArr.map(item => {
        idsArr.push(item.split("~")[0]);
      });
      for (const i in _localCart) {
        for (const j in idsArr) {
          if (_localCart[i].id == idsArr[j]) {
            _localCart.splice(i, 1);
          }
        }
      }
  ...
  async onLoad(options) {
    ...
    const _source = options.source;
    const _stp = options.stp;
    const _groupId = options.groupId;

    if (_source) this.source = _source;
...
.order-create {
  ...
  .order-type-tabs {
    ...
    .order-store-address {
      min-height: 50px;
      padding: 15px;
      .order-store-address-title {
        margin-bottom: 10px;
      }
      .order-store-address-content {
        margin-bottom: 10px;
        .textColor(#333);
        .fontSize(16px);
      }
      .order-store-address-detail {
        margin-bottom: 10px;
      ...
      .order-store-address-footer {
        ...
        .self-mention-time text {
          .textColor(#333);
          .fontSize(16px);
        }
        .self-mention-time-info {
          .fontSize(10px);
          .textColor(#999);
        }
      }

            ...
            <view class="canvas-list">
              <view class="canvas-item" :style="[{zIndex:index2}]" v-for="(iteml,index2) in awardsList" :key="index2">
                <view class="canvas-item-text" :style="[{transform:'rotate('+iteml.turn+')'}]">
                  ...
                  <image class="canvas-item-text-img" src="/static/images/xiaolian.png" v-if="iteml.type == 3"></image>
                  <image class="canvas-item-text-img" src="/static/images/youhuiquan.png" v-if="iteml.type == 2"></image>
                  <image class="canvas-item-text-img" src="/static/images/jingyan.png" v-if="iteml.type == 1"></image>
                  <image class="canvas-item-text-img" src="/static/images/jifenimg.png" v-if="iteml.type == 0"></image>
                </view>
              </view>
            </view>
          </view>
  ...
  data() {
    return {
      awardsConfig: {
        chance: true, //是否有抽奖机会
        lists: [
          {index: 0,id: 11,mold: 1,type: 1,name: "积分1点",amount: "1.00",scale:"0.50"},
          {index: 1,id: 22,mold: 1,type: 1,name: "积分1点",amount: "2.00",scale: "0.10"},
          {index: 2,id: 33,mold: 1,type: 1,name: "积分1点",amount: "3.00",scale: "0.20"},
          {index: 3,id: 44,mold: 1,type: 1,name: "积分1点",amount: "4.00",scale: "0.10"},
      ...
      that
        ...
        .then(function(data) {
          ...
          that.runDeg =
            that.runDeg +
            (360 - that.runDeg % 360) +
            (360 * runNum - awardIndex * (360 / awardsNum.length)) +
            1;
          //创建动画
          if (process.env.VUE_APP_PLATFORM == "h5") {
            ...
            if (document.styleSheets[0].cssRules.length > 0) {
              Array.from(document.styleSheets[0].cssRules).forEach(function(
                element,
                index
              ) {
            ...
            document.styleSheets[0].insertRule(
              "@keyframes rotate_before{from{ transform: rotate(" +
                preDeg +
